...
function validarTelefonoBackend(telefono, paisesActivos) {
    ...
    // Caso 1: Ya viene con + (ej: +5351234567)
    ...
    // Caso 2: Sin + pero podría tener código (ej: 5351234567)
    if (!codigoPais) {
    ...
    // Caso 3: Solo número local (ej: 51234567) - asumir primer país
    ...
    if (!codigoPais || !numeroLocal) {
        return { 
            valido: false, 
            error: `Formato inválido. Use +CódigoPaísNúmero (ej: +${paisesActivos[0]?.codigo}${paisesActivos[0]?.ejemplo}) o solo el número local (${paisesActivos[0]?.digitos} dígitos)` 
        };
    }
