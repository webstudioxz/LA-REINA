...
const ENV = {
    ...
    SESSION_DURATION: parseInt(process.env.SESSION_DURATION) || 86400000, // 24 horas
...
if (!ENV.SUPABASE_URL) ENV_ERRORS.push('❌ SUPABASE_URL no está configurada');
if (!ENV.SUPABASE_SERVICE_ROLE_KEY) ENV_ERRORS.push('❌ SUPABASE_SERVICE_ROLE_KEY no está configurada');

if (ENV_ERRORS.length > 0) {
    console.error('🔴 ERRORES CRÍTICOS DE CONFIGURACIÓN:');
    ENV_ERRORS.forEach(err => console.error(`   ${err}`));
...
console.log(`🔐 ADMIN_PASSWORD: ${ENV.ADMIN_PASSWORD ? '✅ Configurada (segura)' : '❌ NO CONFIGURADA'}`);
...
// ============================================================
// 3. SESIONES EN MEMORIA CON MEJOR PERSISTENCIA
// ============================================================

// ⚠️ CRÍTICO: Usar un Map global para sesiones
...
sessionCleanupInterval = setInterval(cleanupSessions, 300000);
...
setInterval(() => {
    ...
    if (deleted > 0 && ENV.NODE_ENV === 'development') {
        console.log(`🧹 Rate limit limpiado: ${deleted}`);
    }
...
// 5.1 HELMET - Con configuración más permisiva para Render
app.use(helmet({
    ...
    crossOriginEmbedderPolicy: false, // Más permisivo para Render
    crossOriginOpenerPolicy: { policy: "same-origin-allow-popups" },
    crossOriginResourcePolicy: { policy: "cross-origin" }, // Más permisivo para imágenes
...
// 5.2 CORS - Configuración para Render
app.use(cors({
...
// 5.4 RATE LIMITING - Más permisivo para administración
const globalLimiter = rateLimit({
    ...
    skip: (req) => req.path === '/api/admin/login' // Salta el rate limit para login
});

const loginLimiter = rateLimit({
    windowMs: 15 * 60 * 1000,
    max: 20, // Más permisivo para pruebas
...
// ============================================================
// 12. LOGIN ADMIN - CORREGIDO CON LOGS DETALLADOS
// ============================================================
...
app.post('/api/admin/login', (req, res) => {
    ...
    console.log(`🔐 ADMIN_PASSWORD configurada: ${ENV.ADMIN_PASSWORD ? '✅ Sí' : '❌ NO'}`);
    console.log(`🔐 Longitud ADMIN_PASSWORD: ${ENV.ADMIN_PASSWORD ? ENV.ADMIN_PASSWORD.length : 0} caracteres`);
    console.log('🔐 =======================================');
    
    if (!password) {
        console.log('🔴 Login fallido: Contraseña vacía');
    ...
    const passwordConfigurada = ENV.ADMIN_PASSWORD.trim();
    
    console.log(`🔐 Comparación: "${passwordIngresada}" === "${passwordConfigurada}" -> ${passwordIngresada === passwordConfigurada}`);
    
    if (passwordIngresada === passwordConfigurada) {
        const token = crypto.randomBytes(32).toString('hex');
        const expiry = Date.now() + ENV.SESSION_DURATION;
        sessions.set(token, { expiry, ip, createdAt: Date.now() });
        console.log(`✅ Login exitoso desde ${ip} - Token: ${token.substring(0, 8)}...`);
...
app.get('/api/admin/verify-session', (req, res) => {
    ...
    console.log(`🔐 Verificando sesión: ${token ? token.substring(0, 8) + '...' : 'sin token'}`);
    
    if (!token) {
        console.log('🔴 Sesión inválida: Sin token');
        return res.status(401).json({ valid: false });
    }
    
    const session = sessions.get(token);
    if (!session) {
        console.log(`🔴 Sesión inválida: Token no encontrado - ${token.substring(0, 8)}...`);
        return res.status(401).json({ valid: false });
    }
    
    if (session.expiry < Date.now()) {
        console.log(`🔴 Sesión expirada: ${token.substring(0, 8)}...`);
    ...
    console.log(`✅ Sesión válida: ${token.substring(0, 8)}...`);
    res.json({ valid: true });
});
...
app.post('/api/admin/logout', (req, res) => {
    ...
    if (token) {
        sessions.delete(token);
        console.log(`👋 Logout desde ${req.ip} - Token: ${token.substring(0, 8)}...`);
    }
...
const requireAuth = (req, res, next) => {
    ...
    if (!token) {
        console.log('🔴 Auth requerida: Sin token');
        return res.status(401).json({ error: 'No autorizado', message: 'Token de autenticación requerido' });
    }
    
    const session = sessions.get(token);
    if (!session) {
        console.log(`🔴 Auth fallida: Token no encontrado - ${token.substring(0, 8)}...`);
        return res.status(401).json({ error: 'No autorizado', message: 'Sesión inválida' });
    }
    
    if (session.expiry < Date.now()) {
        console.log(`🔴 Auth fallida: Sesión expirada - ${token.substring(0, 8)}...`);
        sessions.delete(token);
...
app.get('/api/admin/config', async (req, res) => {
    try {
        ...
        res.json({
            id: data.id,
            moneda_base: data.moneda_base || 'CUP',
            tasas: data.tasas || { CUP: 1, USD: 0.04, EUR: 0.037 },
...
app.listen(PORT, '0.0.0.0', async () => {
    ...
    console.log(`🔑 Administración: ${ENV.ADMIN_PASSWORD ? '✅ Activada (variable de entorno)' : '❌ DESACTIVADA - Configurar ADMIN_PASSWORD'}`);
