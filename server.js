...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
    ...
    console.error('   Configura estas variables en Render (Environment)');
    process.exit(1);
}

if (!ADMIN_PASSWORD) {
    console.error('❌ ERROR CRÍTICO: Falta ADMIN_PASSWORD');
    console.error('   Configura ADMIN_PASSWORD en Render (Environment)');
    process.exit(1);
}
...
app.get('/api/status', (req, res) => res.json({ online: true, timestamp: new Date().toISOString() }));
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`🗄️ Supabase URL: ${SUPABASE_URL ? '✅ Configurada' : '❌ FALTA'}`);
    console.log(`📸 Bucket: Productos`);
    console.log('========================================\n');
});