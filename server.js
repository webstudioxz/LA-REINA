...
// RUTA DE PAÍSES CORREGIDA
app.get('/api/paises', async (req, res) => {
    const tienda = req.query.tienda;
    console.log(`📞 Solicitando países para tienda: ${tienda}`);
    ...
    try {
        ...
        console.log(`📋 paises_activos desde Supabase:`, data?.paises_activos);
        
        const paisesActivos = data?.paises_activos || ['53'];
        const paisesInfo = paisesActivos.map(codigo => {
            const pais = PAISES_COMPLETOS[codigo];
            if (pais) {
                return {
                    codigo: pais.codigo,
                    nombre: pais.nombre,
        ...
        console.log(`📞 Devolviendo países:`, paisesInfo);
        res.json(paisesInfo);
    } catch (error) {
...
app.post('/api/pedidos', async (req, res) => {
    try {
        ...
        const paisesActivosCodigos = tiendaData?.paises_activos || ['53'];
        const paisesActivos = paisesActivosCodigos.map(c => PAISES_COMPLETOS[c]).filter(p => p);
...
app.post('/api/admin/tiendas', AUTH, async (req, res) => {
    try {
        ...
        console.log('📦 Creando tienda:', { slug, paises_activos: req.body.paises_activos });
        ...
        const { error } = await supabase.from('stores').insert({
            ...
            paises_activos: req.body.paises_activos || ['53'],
            created_at: new Date(),
            updated_at: new Date()
        });
...
app.put('/api/admin/tiendas/:id', AUTH, async (req, res) => {
    try {
        console.log('📦 Actualizando tienda:', { id: req.params.id, paises_activos: req.body.paises_activos });
        ...
        const { error } = await supabase.from('stores').update({
            ...
            paises_activos: req.body.paises_activos,
            updated_at: new Date()
        }).eq('id', req.params.id);
