...
function validarSeguridadNumeroBackend(numero, paisInfo) {
    ...
    if (paisInfo.codigo === '53' && !numero.startsWith('5')) {
        return { valido: false, error: 'En Cuba, los números móviles comienzan con 5' };
    }
...
function validarTelefonoBackend(telefono, paisesActivos) {
    ...
    // Caso 1: Formato con + (ej: +5351234567)
    let match = telefonoLimpio.match(/^\+(\d{1,4})(\d+)$/);
    if (match) {
        codigoPais = match[1];
        numeroLocal = match[2];
    }
    
    // Caso 2: Sin + pero con código de país al inicio (ej: 5351234567)
    if (!codigoPais) {
        for (const pais of paisesActivos) {
            const codigo = String(pais.codigo);
            if (telefonoLimpio.startsWith(codigo)) {
                const posibleNumero = telefonoLimpio.substring(codigo.length);
    ...
    // Caso 3: Solo número local (ej: 51234567) - asumir que es Cuba si está habilitado
    if (!codigoPais && paisesActivos.some(p => p.codigo === '53')) {
        const paisCuba = paisesActivos.find(p => p.codigo === '53');
        if (paisCuba && telefonoLimpio.length === paisCuba.digitos && /^\d+$/.test(telefonoLimpio)) {
            codigoPais = '53';
    ...
    if (!codigoPais || !numeroLocal) {
        return { 
            valido: false, 
            error: `Formato inválido. Use +CódigoPaísNúmero (ej: +5351234567) o solo el número local si es Cuba` 
        };
    }
    
    // Buscar el país en la lista de habilitados
...
app.get('/api/paises', async (req, res) => {
    ...
    try {
        ...
        // Asegurar que paisesActivos es un array
...
app.post('/api/pedidos', validateOrderData, async (req, res) => {
    try {
        ...
        // Obtener países habilitados de la tienda
...
app.post('/api/admin/tiendas', AUTH, async (req, res) => {
    try {
        ...
        // Asegurar que paises_activos es un array
