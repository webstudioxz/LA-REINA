...
if (!ENV.ADMIN_PASSWORD) ENV_ERRORS.push('❌ ADMIN_PASSWORD - ¡CRÍTICO! La contraseña no está configurada');

if (ENV_ERRORS.length > 0) {
    console.error('🔴 ERRORES CRÍTICOS DE CONFIGURACIÓN:');
    ENV_ERRORS.forEach(err => console.error(`   ${err}`));
    if (ENV.NODE_ENV === 'production') {
        console.error('🚫 El servidor NO puede iniciar en producción.');
        console.error('🚫 Configura ADMIN_PASSWORD en las variables de entorno de Render.');
        process.exit(1);
    } else {
