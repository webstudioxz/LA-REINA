...
function validarTelefonoBackend(telefono, paisesActivos) {
    ...
    console.log('🌍 Países activos:', paisesActivos.map(p => p.codigo));
    
    let codigoPais = null;
    let numeroLocal = null;
    
    // INTENTAR EXTRAER CÓDIGO Y NÚMERO
    
    // Caso 1: Formato +XX... (ej: +5351234567)
    const matchConMas = telefonoLimpio.match(/^\+(\d{1,4})(\d+)$/);
    if (matchConMas) {
        codigoPais = matchConMas[1];
        numeroLocal = matchConMas[2];
        console.log('📞 Caso 1 (+): código=', codigoPais, 'número=', numeroLocal);
    }
    
    // Caso 2: Sin + pero empieza con código de país (ej: 5351234567)
    if (!codigoPais) {
        for (const pais of paisesActivos) {
            const codStr = String(pais.codigo);
            if (telefonoLimpio.startsWith(codStr)) {
                const posibleNumero = telefonoLimpio.substring(codStr.length);
                if (posibleNumero.length === pais.digitos && /^\d+$/.test(posibleNumero)) {
                    codigoPais = codStr;
                    numeroLocal = posibleNumero;
                    console.log('📞 Caso 2 (startsWith): código=', codigoPais, 'número=', numeroLocal);
                    break;
                }
            }
        }
    ...
    // Caso 3: Solo número local (asumir primer país)
    if (!codigoPais && paisesActivos.length > 0) {
        const paisDefault = paisesActivos[0];
        if (telefonoLimpio.length === paisDefault.digitos && /^\d+$/.test(telefonoLimpio)) {
            codigoPais = String(paisDefault.codigo);
            numeroLocal = telefonoLimpio;
            console.log('📞 Caso 3 (solo local): código=', codigoPais, 'número=', numeroLocal);
        }
    }
    
    // Caso 4: Extraer código de cualquier parte (último intento)
    if (!codigoPais) {
        for (const pais of paisesActivos) {
            const codStr = String(pais.codigo);
            if (telefonoLimpio.includes(codStr)) {
                const idx = telefonoLimpio.indexOf(codStr);
                const posibleNumero = telefonoLimpio.substring(idx + codStr.length);
                if (posibleNumero.length === pais.digitos && /^\d+$/.test(posibleNumero)) {
                    codigoPais = codStr;
                    numeroLocal = posibleNumero;
                    console.log('📞 Caso 4 (includes): código=', codigoPais, 'número=', numeroLocal);
                    break;
                }
            }
        }
    ...
    if (!codigoPais || !numeroLocal) {
        console.log('❌ No se pudo extraer código de país');
        return { 
            valido: false, 
            error: `Formato inválido. Use +CódigoPaísNúmero (ej: +${paisesActivos[0]?.codigo}${paisesActivos[0]?.ejemplo})` 
        };
    }
    
    // Buscar el país
    const paisInfo = paisesActivos.find(p => String(p.codigo) === String(codigoPais));
    
    if (!paisInfo) {
        console.log('❌ Código no habilitado:', codigoPais);
        return { 
            valido: false, 
            error: `Código de país +${codigoPais} no está habilitado. Códigos disponibles: ${paisesActivos.map(p => `+${p.codigo}`).join(', ')}` 
        };
    }
    
    // Validar longitud
    if (numeroLocal.length !== paisInfo.digitos) {
        console.log('❌ Longitud incorrecta:', numeroLocal.length, 'vs', paisInfo.digitos);
        return { 
            valido: false, 
            error: `El número debe tener ${paisInfo.digitos} dígitos para ${paisInfo.nombre} (ej: ${paisInfo.ejemplo})` 
        };
    }
    
    // Validar seguridad
    const seguridad = validarSeguridadNumeroBackend(numeroLocal, paisInfo);
    if (!seguridad.valido) {
        return { valido: false, error: seguridad.error };
    }
    
    console.log('✅ Teléfono válido:', `+${codigoPais}${numeroLocal}`);
