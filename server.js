...
app.get('/api/config', async (req, res) => {
    try {
        ...
        if (error) {
            ...
            // Configuración por defecto si no existe en Supabase
            const defaultConfig = {
                monedaBase: 'CUP',
                tasas: {
                    CUP: 1,
...
// ============================================
// BUSCAR PEDIDO POR CÓDIGO - NUEVA FUNCIÓN
// ============================================
...
app.get('/api/admin/config', async (req, res) => {
    try {
        ...
        if (error) {
            ...
            // Intentar insertar la configuración por defecto
            const { error: insertError } = await supabase
                .from('config')
                .insert(defaultConfig);
            
            if (insertError) {
                console.log('⚠️ No se pudo crear configuración por defecto');
            }
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`📋 Panel Admin: ${process.env.URL || 'https://la-reina-mgje.onrender.com'}/admin`);
    console.log(`🔍 Búsqueda de pedidos por código: ✅ Activada`);
});