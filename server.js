...
console.log(`ENCRYPTION_MASTER_KEY: ${ENCRYPTION_MASTER_KEY && ENCRYPTION_MASTER_KEY.length >= 32 ? '✅' : '❌'}`);
...
app.use(cors({
    origin: function(origin, callback) {
        ...
        if (allowedOrigins.some(o => origin.match(new RegExp(o.replace(/\*/g, '.*'))))) {
            callback(null, true);
        } else {
...
app.get('/api/tiendas/:id/tarjeta-completa', globalLimiter, async (req, res) => {
    try {
        const { data, error } = await supabase
            .from('store_config')
            .select('tarjeta_encriptada, tarjeta_iv, tarjeta_tag, tarjeta_ultimos_digitos, whatsapp_confirmacion')
            .eq('tienda', req.params.id)
            .single();

        if (error || !data.tarjeta_encriptada) {
            return res.json({ 
                tarjeta: null, 
                configurada: false,
        ...
        res.json({
            ...
            whatsapp: data.whatsapp_confirmacion || '',
...
// ✅ RUTA PUT PARA EDITAR PRODUCTOS
app.put('/api/admin/productos/:id', ADMIN_AUTH, upload.single('imagen'), async (req, res) => {
    try {
        let imagen = req.body.imagen_url || 'https://via.placeholder.com/400';
...
// 11.3 Configuración de tienda (admin)
...
app.post('/api/admin/store-config/:tienda/verificar', ADMIN_AUTH, async (req, res) => {
    try {
        const { data, error } = await supabase
            .from('store_config')
            .select('tarjeta_encriptada, tarjeta_iv, tarjeta_tag, tarjeta_hash, tarjeta_ultimos_digitos, version')
            .eq('tienda', req.params.tienda)
            .single();

        if (error || !data.tarjeta_encriptada) {
            return res.json({ existe: false, mensaje: 'No hay tarjeta configurada' });
        }

        const tarjeta = decryptData(data.tarjeta_encriptada, data.tarjeta_iv, data.tarjeta_tag);
        const hashCalculado = crypto.createHash('sha256').update(tarjeta).digest('hex');
        const integridad = hashCalculado === data.tarjeta_hash;
