...
console.log(`🔍 SUPABASE_URL: ${SUPABASE_URL ? '✅ CONFIGURADO' : '❌ FALTA'}`);
console.log(`🔍 SUPABASE_SERVICE_ROLE_KEY: ${SUPABASE_SERVICE_ROLE_KEY ? '✅ CONFIGURADO' : '❌ FALTA'}`);
console.log(`🔍 ADMIN_PASSWORD: ${ADMIN_PASSWORD ? '✅ CONFIGURADO' : '❌ FALTA'}`);
console.log('🔍 =================================');

if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
    console.error('❌ ERROR CRÍTICO: Faltan variables de Supabase');
    console.error('❌ El servidor NO puede iniciar correctamente');
}
...
app.use(cors({
    origin: [process.env.URL || 'https://la-reina-mgje.onrender.com', 'http://localhost:3000'],
...
app.get('/api/admin/tiendas/:id', async (req, res) => {
    if (!supabase) return res.status(404).json({ error: 'Tienda no encontrada' });
    try {
        const { data, error } = await supabase
            .from('stores')
            .select('*')
...
app.get('/api/admin/categorias', async (req, res) => {
    if (!supabase) return res.json(['otros']);
    try {
        const { data, error } = await supabase
            .from('stores')
            .select('categorias')
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`🔐 Admin: ${ADMIN_PASSWORD ? '✅ Configurado' : '❌ No configurado'}`);
