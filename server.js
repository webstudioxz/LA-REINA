...
async function createSession(token, ip, userAgent) {
...
app.post('/api/admin/login', bruteForceProtection.check, async (req, res) => {
    ...
    if (password === ADMIN_PASSWORD) {
        ...
        const success = await createSession(token, ip, req.get('user-agent'));
...
app.get('/api/admin/inventory/:productId', async (req, res) => {
    try {
        const { data, error } = await supabase
            .from('inventory')
            .select('*')
...
app.post('/api/admin/inventory', validateInventory, async (req, res) => {
    try {
...
// ============================================
// API ADMIN - PRODUCTOS (CON IMÁGENES MÚLTIPLES)
// ============================================
...
app.post('/api/admin/productos', upload.array('imagenes', 10), validateProduct, async (req, res) => {
    try {
...
app.put('/api/admin/productos/:id', upload.array('imagenes', 10), validateProduct, async (req, res) => {
    try {
...
// ============================================
// BÚSQUEDA DE PEDIDOS POR CÓDIGO
// ============================================
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`📋 Panel Admin: ${process.env.URL || 'http://localhost:3000'}/admin`);
    console.log(`📦 Sesiones en Base de Datos: ✅ Activado`);
    console.log(`📊 IPV (Inventory Per Variant): ✅ Activado`);
    console.log(`🖼️ Múltiples imágenes por producto: ✅ Activado`);
});