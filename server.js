...
console.log('🔐 ADMIN_PASSWORD:', ADMIN_PASSWORD ? 'CONFIGURADA (' + ADMIN_PASSWORD.length + ' caracteres)' : 'NO CONFIGURADA');
...
if (!ADMIN_PASSWORD) {
    console.warn('⚠️ ADMIN_PASSWORD no configurada. Usando contraseña por defecto: admin123');
}
...
async function uploadToSupabase(file, folder = 'Productos') {
    try {
        ...
        const { error } = await supabase.storage.from(folder).upload(filePath, file.buffer, {
            cacheControl: '31536000', upsert: false, contentType: file.mimetype
        });
...
async function deleteFromSupabase(imageUrl) {
    try {
        ...
        await supabase.storage.from('Productos').remove([`Productos/${urlParts[1]}`]);
        return true;
    } catch (error) {
...
function generarCodigoUnico() {
    return crypto.randomBytes(6).toString('hex').toUpperCase() + Date.now().toString(36).slice(-4).toUpperCase();
}
...
app.get('/api/tiendas/:id', param('id').isString(), async (req, res) => {
    try {
...
app.get('/api/productos', query('tienda').optional().isString(), async (req, res) => {
    try {
        const tienda = sanitizarTexto(req.query.tienda) || 'electro';
        const { data, error } = await supabase.from('products').select('*').eq('tienda', tienda).eq('disponible', true).order('created_at', { ascending: false });
        if (error) throw error;
        res.json(data || []);
    } catch {
        res.json([]);
...
app.get('/api/categorias', query('tienda').optional().isString(), async (req, res) => {
    try {
        const tienda = sanitizarTexto(req.query.tienda) || 'electro';
        const { data } = await supabase.from('stores').select('categorias').eq('id', tienda).single();
...
app.post('/api/pedidos', strictLimiter, validarPedido, async (req, res) => {
    try {
        ...
        const { error: insertError } = await supabase.from('orders').insert({
            id: nextId, codigo_cliente: codigoCliente, tienda, nombre, telefono, direccion,
            items: req.body.items, total: parseFloat(req.body.total), moneda: 'CUP',
            metodo_pago: metodoPago, estado: 'pendiente',
            created_at: new Date().toISOString(), updated_at: new Date().toISOString()
        });

        if (insertError) {
            if (insertError.code === '23505') return res.status(409).json({ error: 'Conflicto, intente de nuevo' });
            throw insertError;
        }

        await supabase.from('order_counters').upsert({ tienda, counter: nextId });
        res.json({ success: true, orderId: nextId, codigoCliente });
    } catch (error) {
...
app.post('/api/admin/verify', strictLimiter, (req, res) => {
    ...
    console.log('🔐 /api/admin/verify llamado');
    console.log('  - Password recibida:', password ? 'SÍ' : 'NO');
    console.log('  - PASSWORD esperada:', PASSWORD);
...
app.post('/api/admin/categorias', AUTH, async (req, res) => {
    try {
        ...
        if (!currentCats.includes(nuevaCat)) currentCats.push(nuevaCat);
        await supabase.from('stores').update({ categorias: currentCats, updated_at: new Date().toISOString() }).eq('id', req.body.tienda);
        res.json({ success: true });
    } catch {
...
app.put('/api/admin/productos/:id', AUTH, upload.single('imagen'), async (req, res) => {
    try {
        ...
        if (req.file) {
            const { data: oldProduct } = await supabase.from('products').select('imagen').eq('id', req.params.id).single();
...
app.put('/api/admin/pedidos/:id', AUTH, async (req, res) => {
    try {
        await supabase.from('orders').update({ estado: req.body.estado, updated_at: new Date().toISOString() }).eq('id', req.params.id).eq('tienda', req.body.tienda);
        res.json({ success: true });
    } catch {
        res.status(500).json({ error: 'Error' });
    }
...
app.use((err, req, res, next) => {
    ...
    if (err.code === 'LIMIT_FILE_SIZE') return res.status(413).json({ error: 'Archivo muy grande (máx 5MB)' });
