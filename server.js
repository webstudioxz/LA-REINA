...
// ============================================
// RUTAS PÚBLICAS - Servir archivos HTML
// ============================================
app.get('/', (req, res) => {
    ...
    if (require('fs').existsSync(indexPath)) {
        ...
        res.status(404).send('Error: index.html no encontrado. Asegúrate de que el archivo existe en la raíz del proyecto.');
    }
});

app.get('/admin.html', (req, res) => {
    const adminPath = path.join(__dirname, 'admin.html');
    console.log(`Buscando admin.html en: ${adminPath}`);
...
app.get('/api/paises', async (req, res) => {
    ...
    try {
        ...
        const paisesInfo = paisesActivos.map(codigo => PAISES_COMPLETOS[codigo]).filter(p => p);
        res.json(paisesInfo);
    } catch (error) {
        res.json([PAISES_COMPLETOS['53']]);
    }
});
...
app.post('/api/pedidos', async (req, res) => {
    try {
        ...
        const paisesActivos = (tiendaData?.paises_activos || ['53']).map(c => PAISES_COMPLETOS[c]).filter(p => p);
